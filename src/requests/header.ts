...
export const loadCities = () => ajax.get<ResponseBody<{cities: {id: number, name: string}[]}>>({
        url: "/cities",
        credentials: false
    })
    .then(({json, response}) => {
...
export const loadCategories = () => ajax.get<ResponseBody<{categories: {id: number, name: string}[]}>>({
        url: "/categories",
        credentials: false
    })
    .then(({json, response}) => {
